...
// The one verdict a reviewer sends per visit, so this holds one request and no
// queue. The event that is in flight is the state: a dialog with three buttons
// has to disable all three and say which one it is waiting on, and a bare
...
export function useSubmitReview(options: {
  ...
  return { error, pending, reset, submit, submitted };
}
