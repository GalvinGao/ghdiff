...
const POOL_OPTIONS: WorkerPoolOptions = {
  ...
  workerFactory() {
    // The worker entry is ESM. Vite builds a bare `new Worker(url)` as a
    // classic worker, which then rejects the first `import` statement, so the
    // type has to be named here.
    return new Worker(
