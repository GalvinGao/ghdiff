...
export function reviewTargetHref(target: ReviewTarget): string {
  switch (target.kind) {
    case 'github-pull':
      return `/gh/${target.owner}/${target.repo}/pull/${target.number}`;
    case 'github-commit':
      return `/gh/${target.owner}/${target.repo}/commit/${target.sha}`;
    case 'github-compare':
...
export function gitHubTargetFromSegments(
  ...
  if (kind === 'compare') {
    const range = decodeURIComponent(rest.join('/'));
    const parsed = parseCompareRange(range);
